...
export const Calendar: FC<CalendarProps> = ({ startOfWeek, calendarActiveDate }) => {
  ...
  const { data, isLoading, isError, refetch } = useGetEventsForMonthQuery(
    { start: daysArray[0], end: daysArray[41] },
  )
      ...
      {daysArray.map(day =>
        <GridItem
          ...
          <CalendarCell
            ...
            day={day}
            events={data && data.filter(event => moment(event.date)
              .format('DDMMYYYY') === day.format('DDMMYYYY'))} />
        </GridItem>)}
    </Grid>
  )
}
